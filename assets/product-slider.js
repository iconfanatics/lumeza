(function () {
  const initSliders = () => {
    const productSliderSection = document.querySelectorAll(
    ...
    const prevArrows = document.querySelectorAll(
      ".product-slider__buttons .swiper-btn--prev"
    );
    const nextArrows = document.querySelectorAll(
      ".product-slider__buttons .swiper-btn--next"
    );
      ...
      swiperPagination.forEach((element) => {
        if (element.querySelectorAll(".swiper-pagination-bullet").length <= 1) {
          element.parentElement.parentElement
            .querySelector(".product-slider__buttons")
            .classList.add("hide");
        } else {
          element.parentElement.parentElement
            .querySelector(".product-slider__buttons")
  ...
  const scaleImage = () => {
    document.addEventListener("scroll", function () {
      ...
      productSliderSection.forEach((section) => {
        ...
        section
          .querySelectorAll(".product-slider__image-wrapper")
          .forEach((image) => {
            image.style.transform = `scale3d(${
              (100 - (scroll - elOffset) / 60) / 100
            }, ${(100 - (scroll - elOffset) / 60) / 100}, ${
              (100 - (scroll - elOffset) / 60) / 100
            })`;
          });
